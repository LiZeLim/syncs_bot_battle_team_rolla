...
def handle_claim_territory(
    ...
    if len(available) != 0:
        ...
        selected_territory = sorted(
            available, key=lambda x: count_adjacent_friendly(x)
        )[0]

    # Or if there are no such territories, we will pick just an unclaimed one with the greatest degree.
    else:
        selected_territory = sorted(
            unclaimed_territories, key=lambda x: len(game.state.map.get_adjacent_to(x))
        )[0]
...
def handle_attack(
    ...
    strongest_territories = sorted(
        my_territories, key=lambda x: game.state.territories[x].troops, reverse=True
    )
    for territory in strongest_territories:
        move = attack_weakest(
            list(set(game.state.map.get_adjacent_to(territory)) - set(my_territories))
        )
        if move != None:
            return move
...
def handle_troops_after_attack(
    ...
    min_troops_to_move = min(
        move_attack.attacking_troops,
        game.state.territories[move_attack.attacking_territory].troops - 1,
    )
    ...
    move_troops = max(
        ...
        min(
            game.state.territories[move_attack.attacking_territory].troops - 1,
            math.ceil(
                game.state.territories[move_attack.attacking_territory].troops / 2
            ),
        ),
    )
...
def handle_fortify(
    ...
    most_troops_territory = max(
        candidate_territories, key=lambda x: game.state.territories[x].troops
    )

    # To find the shortest path, we will use a custom function.
    shortest_path = find_shortest_path_from_vertex_to_set(
        game,
        most_troops_territory,
        set(game.state.get_territories_owned_by(most_powerful_player)),
    )
    ...
    if (
        len(shortest_path) > 0
        and game.state.territories[most_troops_territory].troops > 1
    ):
        return game.move_fortify(
            query,
            shortest_path[0],
            shortest_path[1],
            game.state.territories[most_troops_territory].troops - 1,
        )
    else:
        return game.move_fortify_pass(query)


def find_shortest_path_from_vertex_to_set(
    game: Game, source: int, target_set: set[int]
) -> list[int]:
